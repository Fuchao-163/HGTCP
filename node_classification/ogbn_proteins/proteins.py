...
def train(model, data, train_idx, optimizer, n, lamda, device):
    model.train()
    criterion = torch.nn.BCEWithLogitsLoss()
    b_xent = torch.nn.BCEWithLogitsLoss()
    optimizer.zero_grad()
    outs, logits = model.global_forward(data.x, data.adj_t, data.distance_matrix, data.nodes_to_community_tensor)
    loss = criterion(outs[train_idx], data.y[train_idx].to(torch.float))

    lbl = torch.unsqueeze(torch.cat((torch.ones(n), torch.zeros(n))), 1).to(device)
...
def test(model, data, split_idx, evaluator):
    ...
    out, _ = model.global_forward(data.x, data.adj_t, data.distance_matrix, data.nodes_to_community_tensor)
...
def main():
    ...
    dataset = PygNodePropPredDataset(
        name='ogbn-proteins', transform=T.ToSparseTensor(attr='edge_attr'), root=args.data_root)
    data = dataset[0]
    ...
    if os.path.exists(f'./distance_{args.num_centroids}_proteins.pkl'):
        ...
        dataset_ = PygNodePropPredDataset(name='ogbn-proteins', root=args.data_root)
        data_ = dataset_[0]
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            loss = train(model, data, train_idx, optimizer, num_nodes, args.lamda, device)
