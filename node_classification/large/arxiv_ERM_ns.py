...
def mask_to_split_idx(data: torch_geometric.data.Data):
    assert hasattr(data, "train_mask")
    return {
        "train": torch.tensor([i for i, e in enumerate(data.train_mask) if e]),
        "valid": torch.tensor([i for i, e in enumerate(data.val_mask) if e]),
        "test": torch.tensor([i for i, e in enumerate(data.test_mask) if e]),
    }

def train(model, loader, x, pos_enc, y, optimizer, device, conv_type, lamda):
    model.train()
    ...
    if conv_type == 'global' : 
        ...
        for edge_index, node_idx, batch_size in loader:
            ...
            feat = x[node_idx] if torch.is_tensor(x) else x(node_idx)
            input = feat.to(device), edge_index.to(device), edge_dist.to(device), pos_enc[node_idx].to(device), node_idx[:batch_size], node_idx
            
            optimizer.zero_grad()
            out, logits = model.to(device)(*input)
...
def test(model, loader, x, pos_enc, y, device, conv_type, fast_eval=False):
    ...
    if conv_type == 'global' : 
        ...
        for edge_index, node_idx, batch_size in loader:
            ...
            edge_index, edge_dist = edge_index[:2], edge_index[2:]
            feat = x[node_idx] if torch.is_tensor(x) else x(node_idx)
            out, _ = model.to(device)(feat.to(device), edge_index.to(device), edge_dist.to(device), pos_enc[node_idx].to(device), node_idx[:batch_size], node_idx)
...
def train_bi(model, loader, x, pos_enc, y, optimizer, device, conv_type, lamda):
    model.train()
...
def main():
    ...
    # data loading
    parser.add_argument('--dataset', type=str, default='ogbn-arxiv', choices=['ogbn-arxiv', 'ogbn-products', 'arxiv-year', 'twitch-gamers', 'pokec', 'genius', 'snap-patents', 'Reddit', 'Flickr'])
    ...
    if args.dataset.startswith('ogbn') :
        ...
        torch_dataset = Reddit(root=f'{data_root}/Reddit')
        ...
        split_idx = mask_to_split_idx(data)
        ...
        torch_dataset = Flickr(root=f'{data_root}/Flickr')
        data = torch_dataset[0]
    ...
    if args.dataset=='Reddit':
        dataset_hd = f'./distance_{args.num_centroids}_r.pkl'
    if args.dataset=='Flickr':
        dataset_hd = f'./distance_{args.num_centroids}_f.pkl'
    ...
    if args.eval :
        ...
        for epoch in range(1, 1 + args.epochs):
            ...
            train_loss, train_acc = train_f(model, train_loader, x, pos_enc, y, optimizer, device, args.conv_type, args.lamda)
