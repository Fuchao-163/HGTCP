...
class VerticalTransformerBlock(MessagePassing):
    ...
    def forward(self, x, x_upper, nodes_to_community_tensor):
        ...
        x_q, x_kv = x_upper, x_bottom

        q = self.lin_query_g(x_q)
        k = self.lin_key_g(x_kv)
        # v = self.lin_key_g(x_kv)
        v = x
...
class TransformerConv(MessagePassing):
    ...
    def __init__(
        ...
        num_centroids: Optional[int] = None,
        # centroid_dim: int = 64,
        nodes_to_community_tensor = None,
        ...
        **kwargs,
        ...
        if self.conv_type != 'local' :
            self.vq = VectorQuantizerEMA(
                ...
                global_dim, 
                decay=0.99,
                nodes_to_community_tensor = nodes_to_community_tensor
            )
        ...
        self.lin_Vertical = Linear(heads * out_channels, global_dim)
        self.convs_Vertical=VerticalTransformerBlock(
                in_channels=global_dim*2,
                out_channels=global_dim*2,
                global_dim=global_dim,
                heads=heads,
                dropout=dropout,
                num_centroids=num_centroids,
            )
        self.nodes_to_community_tensor = nodes_to_community_tensor
        self.global_dim = global_dim

        self.disc = Discriminator(out_channels, 1)
    ...
    def forward(self, x: Tensor, edge_index: Adj, edge_attr: OptTensor = None, 
                    pos_enc=None, batch_idx=None, node_idx=None):
        ...
        if self.conv_type == 'local' :
            ...
            out_local_all = self.local_forward(x, edge_index, edge_attr)
            out_local = out_local_all[:len(batch_idx)]

            x_upper = self.vq.get_k()

            x_bottom = torch.cat([self.lin_Vertical(out_local_all), pos_enc], dim=1)
            x_upper = self.convs_Vertical(x_bottom, x_upper, self.nodes_to_community_tensor[node_idx])

            k_x = x_upper
            v_x = x_upper[:, :self.global_dim]


            out_global = self.global_forward(x[:len(batch_idx)], k_x, v_x, pos_enc[:len(batch_idx)], batch_idx)
            # out = torch.cat([out_local, out_global], dim=1)

            # contrastive learning
            ret = self.disc(out_global, out_local)

            out = (1 - self.alpha_cl) * out_global + self.alpha_cl * out_local
    ...
    def global_forward(self, x, k_x, v_x, pos_enc, batch_idx):

        distance_matrix = self.distance_matrix[batch_idx]
        ...
        if self.training :
            x_idx = self.vq.update(q_x, batch_idx)
...
class Transformer(torch.nn.Module):
    def __init__(self, num_nodes, in_channels, hidden_channels, out_channels, global_dim, num_layers, heads, ff_dropout, attn_dropout, spatial_size, skip, dist_count_norm, conv_type,num_centroids, no_bn, norm_type, distance_matrix, nodes_to_community_tensor, alpha_cl):
        ...
        for _ in range(num_layers):
            ...
            h_times = 1 if conv_type == 'full' else 1
    ...
    def forward(self, x, edge_index, edge_attr, pos_enc, batch_idx, node_idx):
        x = self.fc_in(x)
        for i, conv in enumerate(self.convs):
            x, logits = conv(x, edge_index, edge_attr, pos_enc, batch_idx, node_idx)
            x = self.ffs[i](x)
        x = self.fc_out(x)
        return x, logits
