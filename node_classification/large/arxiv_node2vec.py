...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='ogbn-arxiv',
                        choices=['ogbn-arxiv', 'ogbn-products', 'arxiv-year', 'twitch-gamers', 'pokec', 'genius',
                                 'snap-patents', 'Reddit', 'Flickr'])
    ...
    if args.dataset == 'ogbn-arxiv' :
        dataset = PygNodePropPredDataset(name='ogbn-arxiv', root=args.data_root)

        data = dataset[0]
        data.edge_index = to_undirected(data.edge_index)
    elif args.dataset == 'Reddit' :
        torch_dataset = Reddit(root=f'{args.data_root}/Reddit')
        data = torch_dataset[0]

        data.edge_index = to_undirected(data.edge_index)
    elif args.dataset == 'Flickr' :
        torch_dataset = Flickr(root=f'{args.data_root}/Flickr')
        data = torch_dataset[0]
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for i, (pos_rw, neg_rw) in enumerate(loader):
            ...
            if (i + 1) % 100 == 0:  # Save model every 100 steps.
                save_embedding(model, args.embedding_dim, args.dataset)
