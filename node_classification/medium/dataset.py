...
from torch_geometric.datasets import Planetoid, Reddit, Flickr, PPI, Amazon, Yelp
...
def load_nc_dataset(args):
    ...
    DATAPATH = os.path.join(args.data_dir, '')
    ...
    elif dataname in ('Photo', 'Computers'):
        dataset = load_amazon_dataset(dataname)
    elif dataname in ('Reddit'):
        dataset = load_reddit_dataset(dataname)
    elif dataname in ('Flickr'):
        dataset = load_flickr_dataset(dataname)
    elif dataname in ('Yelp'):
        dataset = load_yelp_dataset(dataname)
    else:
        raise ValueError('Invalid dataname')
    dataset.graph['data_root'] = DATAPATH
    return dataset

def load_yelp_dataset(name):
    torch_dataset = Yelp(root=f'{DATAPATH}Yelp')
...
def load_flickr_dataset(name):
    torch_dataset = Flickr(root=f'{DATAPATH}Flickr')
    data = torch_dataset[0]
...
def load_reddit_dataset(name):
    torch_dataset = Reddit(root=f'{DATAPATH}Reddit')
    data = torch_dataset[0]
...
def load_amazon_dataset(name):
    torch_dataset = Amazon(root=f'{DATAPATH}Amazon', name=name)
    data = torch_dataset[0]
...
def load_wiki_new(name, no_feat_norm=False):
    path=os.path.join(DATAPATH, f'geom-gcn/{name}/{name}_filtered.npz')
    data=np.load(path)
    # lst=data.files
    # for item in lst:
    #     print(item)
