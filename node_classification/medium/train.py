...
print(f'train size={len(split_idx_lst[0]["train"])}, valid size={len(split_idx_lst[0]["valid"])}, test size={len(split_idx_lst[0]["test"])}')
...
for run in range(args.runs):
    ...
    if args.dataset in ['cora', 'citeseer', 'pubmed', "Reddit", "Flickr", "Yelp"] and args.protocol == 'semi':
    ...
    for epoch in range(args.epochs):
        ...
        if result[1] > best_val:
            best_val = result[1]
            patience = 0
            # torch.save(model.state_dict(), f"./checkpoint/{args.dataset}_model_weights_{run}.pth")
        else:
            patience += 1
        ...
        if epoch % args.display_step == 0:
            ...
            # print(f'Train Mem:{torch.cuda.max_memory_allocated(device=device)/1e6:.2f} MB')
