...
def load_fixed_splits(dataset, name, protocol):
    ...
    if name in ["cora", "citeseer", "pubmed"] and protocol == "semi":
    ...
    elif name in ["Reddit", "Flickr", "Yelp"] and protocol == "semi":
        splits = {}
        splits["train"] = torch.as_tensor(dataset.graph['split_idx']['train'])
        splits["valid"] = torch.as_tensor(dataset.graph['split_idx']['valid'])
        splits["test"] = torch.as_tensor(dataset.graph['split_idx']['test'])
        splits_lst.append(splits)
    elif name in ["chameleon", "squirrel"]:
        file_path = os.path.join(dataset.graph['data_root'], 'geom-gcn', name,
                                 f'{name}_filtered.npz')
        ...
        for i in range(10):
            splits_file_path = (
                os.path.join(dataset.graph['data_root'], "geom-gcn/splits/{}".format(name))
                + "_split_0.6_0.2_"
                + str(i)
                + ".npz"
            )
            ...
            with np.load(splits_file_path) as splits_file:
                ...
                splits["train"] = torch.where(torch.BoolTensor(splits_file["train_mask"]))[0]
                splits["valid"] = torch.where(torch.BoolTensor(splits_file["val_mask"]))[0]
                splits["test"] = torch.where(torch.BoolTensor(splits_file["test_mask"]))[0]
            splits_lst.append(splits)

    elif name in ['deezer-europe']:
        splits_lst = np.load(os.path.join(dataset.graph['data_root'], 'deezer',
                                          f'{name}-splits.npy'), allow_pickle=True)
